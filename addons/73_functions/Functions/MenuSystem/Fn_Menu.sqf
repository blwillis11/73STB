...
_highestCard = "No Security Card";
{
	...
	if (true /*(format ["DMNS_UNSC_SecurityCard_%1",_x]) in (magazines player)*/) then {
...
DMNS_UNSC_Console_MenusAllowed = [];
{
	_value = getNumber (configfile >> "cfgDmnsUNSCmenus" >> _x >> "Security");
	if (_value in _cardValues) then {
		DMNS_UNSC_Console_MenusAllowed pushBack _x;
	};
} forEach _arguments;

_openThis = DMNS_UNSC_Console_MenusAllowed select 0;
_function = getText	(configfile >> "cfgDmnsUNSCmenus" >> _openThis >> "Function");

DMNS_CurrentConsole = _console;
DMNS_CurrentConsoleString = _highestCard;
...
if (_function != "") then {
	...
	createDialog (getText (configfile >> "cfgDmnsUNSCmenus" >> _openThis >> "Dialog"));
};
