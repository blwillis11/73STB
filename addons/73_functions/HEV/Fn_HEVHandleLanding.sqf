...
[
	...
	{
		((getPosATL (_this select 0)) select 2 < 1) or {
			((getPosATL (_this select 0)) select 2 < (_this select 1)) and ((velocity (_this select 0)) select 2 isEqualTo 0)
		} or {
			(getPosASLW (_this select 0) select 2) <= 3
		}
	},
	{
