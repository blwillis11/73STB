...
if (not _held) exitWith {
    ...
    if(not isTouchingGround player and (vehicle player == player)) then {
        waitUntil{
            uiSleep .1;
            [(getPosASL player),velocity (vehicle player)] call STB73_JumpPack_fnc_RoofStuckCheck;
...
[] call STB73_JumpPack_fnc_EventHandlers;
...
while {STB73_JumpPack_ON} do {
    ...
    _yInc = (_vel select 2);
    if(!([(getPosASL player),_vel] call STB73_JumpPack_fnc_RoofStuckCheck)) then {
        if (not (isTouchingGround player)) then {
            if(_yInc < 0) then {
                // Half Fuel & Heat Changes
	            _newHeat = _heat + 0.005;
	            _newFuel = _fuel - 0.0015;
                if(_yInc < -1.5) then {
...
uiSleep 1;
[] spawn STB73_JumpPack_fnc_JumpPackRefuel;