...
_groupSpeciality =  [_AssaultTroopsArray, 2, "AT"] call BIS_fnc_param// _groupSpeciality = _podArrayString select 3;*/
	...
	_style = [_this, 4, "10th MEB"] call BIS_fnc_param;
...
// _everyThingSpawned = units _pelicanGroup + [_pelican]; 
	{
		_x setVariable ["OPTRE_AllSpawned", (units _pelicanGroup + [_pelican]), false]
...
// End if Event Handlers Pelican. 
	{
		_x addEventHandler ["Killed", {
			if isServer then {
				...
				{
					_x addCuratorEditableObjects [(( (_this select 0) getVariable ["OPTRE_AllSpawned", []] ) + [_veh]), true];
				} forEach allCurators;
			};
		}];
	} forEach [_pelican, driver _pelican];
...
// spawn group and vehicle in / attachTo Pelican 
	...
	if _isVehicleDrop then {
		...
		if (vehicle (_crew select 1) == (_crew select 1)) then {
			deleteVehicle (_crew select 1);
		};
		_veh call _code;
...
{
	_x assignAsCargo _pelican;
	_x moveInCargo _pelican;
...
{
	_x setUnitAbility (0.5 + (random 0.5));
} forEach (units _group2 + units _group1);

{
	...
	if (str _MarkerPos != "[0, 0, 0]") then {
		...
		if _isVehicleDrop then {
			_VehWPPosArray pushBack ([_MarkerPos, 100, 270] call OPTRE_fnc_MathsTriangulatePos);
		};
	};
...
switch (_endWaypoint) do {
	...
	case "garrison": {
		{
			_wpX = _x addWaypoint [(getMarkerPos (_wayPoints select -1)), 0];
...
_allUnits = if _isVehicleDrop then {
	[_veh] + _crew + units _group1 + units _group2
} else {
	units _group1 + units _group2
};
if _isVehicleDrop then {
...
_wpUnLoad setWaypointStatements ["vehicle this land 'GET OUT';
	true",
	"
	if isServer then {
		_pelican = vehicle this;
...
// Return
_return = if _isVehicleDrop then {
	[_crewGroup, _group1, _group2]
} else {
	[_group1, _group2]
};
_return