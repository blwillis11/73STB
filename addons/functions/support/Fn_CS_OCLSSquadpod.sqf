	...
	if (!isServer) then {
		...
		waitUntil {
			if (alive _mainprojectile) then {
				_position = getPosATL _mainprojectile
			};
		...
		[{
			_p = _this#0;
			if (alive _p) then {
				deleteVehicleCrew _p;
				deleteVehicle _p;
			}
		}, [_projectile], 60] call CBA_fnc_waitAndExecute;
		...
		if (count _wayPoints > 0 AND _endWaypoint != "") then {
			switch (_endWaypoint) do {
				case "cycle": {
					private _wayPointX = _EliteUnit addWaypoint [(getMarkerPos (_wayPoints select -1)), 0];
