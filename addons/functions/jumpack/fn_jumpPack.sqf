params [["_held", false, [false]]];
private ["_velC", "_vel", "_pos", "_height", "_arrayDir", "_dir", "_addDir"];

_backpack = backpack player;
if ((_backpack != "STB73_JumpPack_on" AND _backpack != "STB73_JumpPack")) exitWith {};
if !(STB73_JumpPack_Enabled) exitWith {
	hint "JumpPacks are currently disabled by an admin";
};

if ((player getVariable ["ace_medical_isUnconscious", false]) or (not (alive player)) or (!isNull objectParent player)) exitWith {};
...
if (not _held) exitWith {
	...
	if (not (isNil "STB73_JumpPack_Effects")) then {
		{
			deleteVehicle _x;
		} forEach STB73_JumpPack_Effects;
	};
	...
	if (!isNil "STB73_JumpPack_keyDownEH") then {
		(findDisplay 46) displayRemoveEventHandler ["KeyDown", STB73_JumpPack_keyDownEH];
	};
