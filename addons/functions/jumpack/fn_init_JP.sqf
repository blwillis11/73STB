...
if (hasInterface) then {
	[] spawn {
		waitUntil {
			!isNil "CBA_fnc_addKeybind"
		};
		waitUntil {
			!isNull (findDisplay 46)
		};

		if (missionNamespace getVariable ["STB73_JumpPack_KeybindsRegistered", false]) exitWith {};
		missionNamespace setVariable ["STB73_JumpPack_KeybindsRegistered", true];
		...
		["73rd Jumpack", "STB73_JumpPack_Toggle", ["Jump Pack Toggle", "Press this key to toggle activate the JumpPack"], {
			[!(missionNamespace getVariable ["STB73_JumpPack_ON", false])] spawn STB73_fnc_JumpPack;
