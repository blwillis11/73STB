...
_podClassname = if (count _podArray >= _arrayPosition + 1) then {
	(typeOf (_podArray select _arrayPosition))
} else {
	""
};

{
	...
	if (_podClassname == (getText (configFile >> "CfgVehicles" >> "Module_OPTRE_PelicanSupplyDrop" >> "Arguments" >> "box2" >> "values" >> _x >> "value"))) then {
